...
singleton pages (`homePage`, `aboutPage`, `contactPage`), the reusable `world`
collection, and the `photo` document library. Part of the
[FrameOS monorepo](../../README.md); see
...
| `schema:extract` | Export the Studio schema for TypeGen             |
| `typegen`        | Regenerate typed GROQ results in the web package |
...
- `schemaTypes/documents/` — `siteSettings`, `homePage`, `aboutPage`,
  `contactPage`, `world`, `photo`
- `schemaTypes/objects/` — reusable field groups: `seo`, `cta`, `socialLink`
- `migrations/` — idempotent content migrations; run a dry run before applying
