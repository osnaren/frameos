...
- Keep import ordering and grouping compatible with `apps/web/eslint.config.js` (`import/order` is enforced).
- Preserve formatting conventions from the root `prettier.config.js` (single quotes, no semicolons, width 100) — `apps/studio` has its own override (bracketSpacing: false) via its `package.json`'s embedded `prettier` key.
- Naming:
...
- Environment parsing is centralized in `apps/web/src/server/env.ts`; add new server env vars there with zod validation.
...
- Gallery filtering/search normalization must use `apps/web/src/lib/gallery-search.ts`.
- Photo slugs are Sanity's native `slug` field (`photo.slug.current`) — there is no separate slug-encoding utility.
- Keep caching and invalidation behavior consistent with `apps/web/src/server/cache/*` and repository reconciliation flows.

## API, Webhook, and Reconciliation Rules

- Webhook endpoints (`apps/web/src/routes/api/webhooks/*.ts`) must preserve:
  - signature verification
  - idempotency checks
  - stale-event protection
    before reconciliation.
- Reconcile endpoint (`apps/web/src/routes/api/reconcile/content.ts`) requires token auth behavior compatible with `RECONCILE_SECRET` / `CRON_SECRET`.

## Key References (link, don’t embed)

- `README.md`
- `docs/README.md` (full architecture, guides, and decision records)
- `package.json` (root, workspace scripts) / `apps/web/package.json` / `apps/studio/package.json`
- `apps/web/eslint.config.js`
- `prettier.config.js`
- `apps/web/src/styles.css`
- `apps/web/src/routes/archive.tsx`
- `apps/web/src/routes/photos.$slug.tsx`
- `apps/web/src/server/server-functions/portfolio.ts`
- `apps/web/src/server/repository/portfolio.ts`
- `apps/web/src/routes/api/webhooks/sanity.ts`
